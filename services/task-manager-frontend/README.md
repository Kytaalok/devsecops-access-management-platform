...
По умолчанию фронтенд использует `Base URL = /api` и проксирует запросы через Nginx в контейнер `api`.
...
- `/api` (рекомендуется для запуска через Docker Compose)
- `http://localhost:8000` (при прямом обращении к API)
